    ...
    >
      <svg
        ...
        className={cn(
            ...
            editMode === 'move' ? "cursor-move" : editMode === 'delete' ? "cursor-not-allowed" : "cursor-crosshair"
        )}
        ...
        <g ref={gRef}>
          ...
          {editMode === 'connect' && startStation && (
              <line
                ...
                x2={mousePos.x}
                y2={mousePos.y}
                stroke={selectedLineId ? lines[selectedLineId]?.color : "hsl(var(--primary))"}
                strokeWidth="4"
                strokeDasharray="8 4"
                ...
                return (
                    <g key={edge.id} className="edge-group">
                        ...
                        <line
                            ...
                            style={{ 
                                opacity: (selectedLineId && !isLineActive) || (editMode === 'delete' && !isSelected) ? 0.2 : 1 
                            }}
                        />
                    </g>
                )
          ...
          <g className="stations-layer" ref={stationsLayerRef}>
            {stationsArray.map((station) => {
              const belongingLineIds = [...new Set(edgesArray.filter(e => e.station1Id === station.id || e.station2Id === station.id).map(e => e.lineId))];
              const primaryLineId = belongingLineIds[0];
              const stationColor = primaryLineId ? (lines[primaryLineId]?.color || "#333") : "#ccc";
              
              const isSelected = selectedStationId === station.id;
              const isConnectionStart = connectionStartId === station.id;

              const numberings = station.numbering.split(/[\s,]+/).filter(Boolean);
              ...
              return (
                ...
                >
                  ...
                  <rect
                    ...
                    fill="white"
                    stroke={isSelected ? "hsl(var(--primary))" : belongingLineIds.length > 1 ? "#333" : stationColor}
                    ...
                    {numberings.map((num) => {
                        ...
                        return (
                            <text
                                ...
                                style={{ fill: '#333' }}
                            >
                                <tspan x={numberings.indexOf(num) * 24} y="-2" style={{ fontSize: '10px' }}>{alpha}</tspan>
                                <tspan x={numberings.indexOf(num) * 24} y="10" style={{ fontSize: '12px' }}>{digit}</tspan>
                            </text>
                        );
                    })}
                  </g>
      ...
      <div className="absolute top-4 left-4 p-3 bg-background/80 backdrop-blur border rounded-md shadow-sm pointer-events-none text-xs space-y-1">
        <div className="font-semibold text-primary mb-1">Tokyo Metro Style Editor</div>
      ...
      {showLegend && linesArray.length > 0 && (
        <div className="absolute bottom-4 right-4 p-4 bg-background/80 backdrop-blur border rounded-md shadow-sm pointer-events-none text-xs min-w-[120px] max-h-[300px] overflow-hidden flex flex-col gap-2">
            <div className="font-semibold text-primary border-b pb-1.5 mb-1 tracking-tight">凡例 (Legend)</div>
            <div className="space-y-2 overflow-y-auto">
                {linesArray.map(line => (
                   <div key={line.id} className="flex items-center gap-2">
                       <div className="size-3 rounded-full shrink-0 shadow-sm" style={{ backgroundColor: line.color }} />
